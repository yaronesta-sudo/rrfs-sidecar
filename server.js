...
async function findLatestRun() {
  const now = new Date();
  for (let lookback = 0; lookback <= 6; lookback++) {
    ...
    const probe =
      `${S3_BASE}/rrfs_a/rrfs.${day}/${hh}/rrfs.t${hh}z.prslev.f001.conus.grib2.idx`;
...
async function fetchTmp2mRecord(day, hh, fhr) {
  ...
  const baseUrl = `${S3_BASE}/rrfs_a/rrfs.${day}/${hh}/rrfs.t${hh}z.prslev.f${fhrStr}.conus.grib2`;
