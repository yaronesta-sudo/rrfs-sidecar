RRFS Sidecar
> **v11**: built on `sondngyn/wgrib2:latest` — wgrib2 ships prebuilt, no compile step.
...
Create a new GitHub repo (e.g. `rrfs-sidecar`) and push only this folder's contents to its root.
On render.com:
New + → Web Service
...
Wait ~5 minutes for the first build. When it shows "Live", copy the URL
...
Paste the URL into Lovable when prompted to set the `RRFS_SIDECAR_URL` secret.
Cold-start behavior
